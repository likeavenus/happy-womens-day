...
class Game extends Phaser.Scene {
  ...
  create() {
    ...
    this.fish.setCollidesWith(this.collisionCategory2 | this.collisionCategory1);
    /** music */
    ...
    this.hutBody = this.matter.add.rectangle(this.hut.x, this.hut.y + 60, this.hut.width * 2, this.hut.height, {
      isStatic: true,
    });
    ...
    this.lever = this.matter.add
      .image(X - Math.cos(this.tweener.x) * LEVER, Y - Math.sin(this.tweener.x) * LEVER, null, null, {
        isSensor: true,
        isStatic: true,
      })
      .setVisible(false);
    ...
    eKey?.on("down", (e) => {
      ...
      this.tweens.add({
        ...
        onStart: () => {
          this.lizard.setCollidesWith(this.collisionCategory3 | this.collisionCategory2 | this.collisionCategory1);
          this.rod.at(-2).setCollisionCategory(this.collisionCategory3);
          this.rod.at(-2).setCollidesWith(this.collisionCategory1);
        },
        onComplete: () => {
          this.lizard.setCollidesWith(this.collisionCategory2 | this.collisionCategory1);
          this.fish.visible = false;
          this.rod.at(-2).setCollisionCategory(this.collisionCategory3);
          this.rod.at(-2).setCollidesWith(this.collisionCategory1 | this.collisionCategory2);
        },
      });
    });
    ...
    const tileset2 = map2.addTilesetImage("floor", "tiles-floor") as Phaser.Tilemaps.Tileset;
    ...
    groundLayer2?.setCollidesWith(this.collisionCategory1 | this.collisionCategory3 | this.collisionCategory4);

    groundLayer3?.setCollisionCategory(this.collisionCategory2);
    groundLayer3?.setCollidesWith(this.collisionCategory1 | this.collisionCategory3 | this.collisionCategory4);
    ...
    this.matter.world.on("collisionstart", (e, bodyA, bodyB) => {
      ...
      // );
      if (e.pairs.some((pair) => pair.bodyA.label == "water" && pair.bodyB.label == "hook")) {
        const i = this.waterBody.columns.findIndex((col, i) => col.x + 370 >= bodyB.position.x && i);

        const speed = bodyB.speed * 3;
        const numDroplets = Math.ceil(bodyB.speed) * 6;

        // bodyB.setFrictionAir(0.25);
        this.waterBody.splash(Phaser.Math.Clamp(i, 0, this.waterBody.columns.length - 1), speed, numDroplets);
      }
      if (e.pairs.some((pair) => pair.bodyA.label == "lizard" && pair.bodyB.label == "water")) {
        const i = this.waterBody.columns.findIndex((col, i) => col.x + 370 >= bodyA.position.x && i);
    ...
    this.matter.world.on("collisionactive", (e, o1, o2) => {
      ...
      if (e.pairs.some((pair) => pair.bodyA.label == "lizard" && pair.bodyB.label == "hook")) {
  ...
  update(time: number, delta: number) {
    ...
    if (this.rod.at(-2).inWater) {
      ...
      if (randomEventId === 5 && this.starsSummary > 0) {
        lastRod.setPosition(lastRod.x + (Math.random() - 2.5) * 5, lastRod.y + 5);
      }
    }
  }
  ...
  createRod() {
    ...
    for (var i = 0; i < count; i++) {
      ...
      const newItem = this.matter.add
        .image(this.lizard.x + i * 15, this.lizard.y + i * 2, isLastItem ? "hook" : "rope", null, {
          shape: "circle",
      ...
      if (isPrevLastItem) {
        ...
        newItem.setCollidesWith(this.collisionCategory1 | this.collisionCategory2);
      }
  ...
  punchLizard(body, lizards?: Phaser.Types.Physics.Arcade.ImageWithDynamicBody) {}
}
